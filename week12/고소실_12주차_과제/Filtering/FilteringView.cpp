...
void CFilteringView::GaussianFilteringCPU() {
	...
	for (r = 0; r < m_height; r++) {
		for (c = 0; c < m_width; c++) {
			imageBitsFiltered[r * m_width + c] = 0;
			mean = 0;
			for (l = -w; l <= w; l++) {
				for (k = -w; k <= w; k++) {
					if ((r + l) < 0 || (r + l) >= m_height || (c + k) < 0 || (c + k) >= m_width) continue;
					else
						mean += imageBits[(r + l) * m_width + c + k] * kernel[(l + w) * w0 + k + w];
				}
			}
...
void CFilteringView::MovingAverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
		//	m_mfcImage1->SetPixel(c,r,RGB(m_imageBits[r*m_width+c],m_imageBits[r*m_width+c],m_imageBits[r*m_width+c]));
